...
captured <- details[status == 'captured', .(piece, to, number_move_capture)]
setnames(captured, old = c("number_move_capture"), new = c("turn"))

end_game <- details[status == 'captured', .(piece, from, to, number_move)]
end_game[, to := ifelse(is.na(to), from, to)][, from := NULL]
setnames(end_game, old = c("number_move_capture"), new = c("turn"))
