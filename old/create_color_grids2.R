...
# This function builds out a grid and returns either
# the size of the grid or
# 0 if anything fails
...
# This loops over luminance values and size values to return the largest grid
# We don't need the same luminance value for all hue and chroma values, so
# each hue/chroma value has to be checked individually
...
# Just returns the size instead of the grid
...
# Loop over a wide but sparse set of inputs, save outputs
t1 <- Sys.time()
...
## Now maximize on a finer grain again for
...
# Save max size and max c_value, any h_angle
